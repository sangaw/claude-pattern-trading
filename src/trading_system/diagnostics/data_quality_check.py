...
class DataQualityCheck:
    ...
    def run_data_quality_check(self):
        ...
        if total_missing > 0:
            ...
            if total_missing / len(self.df) > 0.05:
                print("\n WARNING: >5% missing values - data quality may be poor")
            else:
    ...
    def impute_missing_values(self, strategy='finance_standard'):
        """
        Impute missing values in a financial dataset.
        
        Args:
            df (pd.DataFrame): The input dataframe.
            method (str): 'ffill' (forward fill), 'interpolate' (linear), or 'drop'.
            fill_initial_indicators (bool): If True, fills NaNs at the start 
                                            (warmup period) with 0 or first value.
        Returns:
        ...
        if strategy == 'finance_standard':
            # Forward fill price gaps
            df_clean = df_clean.ffill()
            # Drop rows that are still NaN (indicator warmup)
            # df_clean = df_clean.dropna()
            
        # elif strategy == 'zero_fill':
        #   df_clean = df_clean.fillna(0)
        
        print(f"✓ Imputation complete. {initial_missing} missing values handled.")
