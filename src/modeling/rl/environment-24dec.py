...
if RL_AVAILABLE:
    class PatternAwareTradingEnv(gym.Env):
        ...
        def step(self, action):
           ...
           # Ensure terminated triggers at the very last bar of your dataframe
            terminated = bool(self.idx >= len(self.df) - 1) 

            # Ensure truncated triggers when you hit the limit you set (e.g., 300)
            ...
            # 3. Action Execution (Hold: 0, Long: 1, Flatten: 2)
            if action == 1:  # GO LONG
                if self.position == 0:
                    self.position = 1
                ...
                if self.position == 1:
                    ...
                    capped_return = np.clip(trade_return, -0.1, 0.1) # Safe clipping
                    ...
                    reward -= 0.001 # Penalty for invalid flatten
            
            # 4. Mark-to-Market (The source of the "Life" in your graph)
            if self.position == 1:
                price_change = (curr_close - prev_close) / prev_close if prev_close != 0 else 0
                
                # Risk-adjusted continuous reward
                reward += (price_change / (volatility + 0.01)) * 0.5
                
                # Update Equity for the dashboard
                unrealized_pnl_pct = (curr_close - self.entry_price) / self.entry_price
                self.equity = self.initial_capital * (1 + unrealized_pnl_pct)
            else:
                # If flat, equity stays at the initial capital level
                # Note: In a more complex env, you'd track cumulative equity
            ...
            # 6. Unified Info Dictionary (Matched to your class variables)
            action_names = {0: "Hold", 1: "Long", 2: "Flatten"}
            info = {
                'portfolio_value': float(self.equity), # Use self.equity, NOT self.portfolio
